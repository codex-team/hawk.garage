<template>
  <div
    class="entity-image"
    :class="{
      'entity-image--placeholder': isPlaceholder
    }"
    :style="style"
  >
...
export default {
  ...
  props: {
    ...
    /**
     * Image size.
     * Font size will be calculated from this value
     */
    size: {
      type: [Number, String],
      default: 36,
  ...
  computed: {
    /**
     * If we haven't email and image, we will show icon-placeholder instead of abbreviation
     *
     * @returns {boolean}
     */
    isPlaceholder() {
    ...
    style() {
      ...
      css.backgroundImage = this.isImageShowing ? `url('${this.image}')` : 'none';
      css.backgroundColor = this.bgColor;
      ...
      if (!this.isPlaceholder && this.name) {
        ...
        /**
         * If there is one char in abbreviation, make it little bigger
         */
        if (isSingleCharAbbreviation) {
          css.fontSize = `${this.size / 1.7}px`;
        } else {
...
<style>
  .entity-image {
    ...
    @supports(-webkit-clip-path: polygon(0 0, 100% 0, 100% 75%, 0 100%)) or (clip-path: polygon(0 0, 100% 0, 100% 75%, 0 100%)){
      ...
      /**
       * Prevent rendering bug in Chrome
       */
