...
import { createIncrementalLexer } from './markdown';
...
const TRICKY = {
  ...
  'a definition right after its reference': 'Read [docs] carefully.\n\n[docs]: https://hawk.so/docs\n',
  'a realistic answer': [
...
describe('createIncrementalLexer', () => {
  ...
  it('should read a definition into a settled reference on reparse', async () => {
    ...
    for (const character of source) {
      streamed = lexer.append(character);
    }

    expect(shape(streamed)).not.toBe(shape(marked.lexer(source)));
    expect(shape(lexer.reparse())).toBe(shape(marked.lexer(source)));
  });
