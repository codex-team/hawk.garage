...
export function renderMarkdown(text: string): string {
  ...
  const applyInline = (s: string): string => {
    ...
    s = s.replace(/`([^`]+)`/g, (_m, p1) => {
      ...
      codeSlots.push(`<code>${p1}</code>`);
      return `\u0000CODE${idx}\u0000`;
    });

    // Links: [text](url)
    s = s.replace(/\[([^\]]+)\]\(([^)\s]+)\)/g, (_m, textValue, href) => {
      return `<a href="${sanitizeUrl(href)}" target="_blank" rel="noopener noreferrer">${textValue}</a>`;
    });
    ...
    // Restore code slots
    s = s.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) => codeSlots[Number(i)]);

    return s;
  };

  const escaped = escape(text || '') as string;
  ...
  while (i < lines.length) {
    ...
    if (/^&gt;\s?/.test(line)) {
      ...
      html.push(`<blockquote>${applyInline(block.join('<br/>'))}</blockquote>`);
      continue;
    }
    // Unordered list
    if (/^\s*([-+*])\s+/.test(line)) {
